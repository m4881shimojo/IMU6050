...
{
  /* USER CODE BEGIN 1 */
	MPU_ConfigTypeDef myMpuConfig; // ジャイロセンサの特性を決めるパラメータを設定
  /* USER CODE END 1 */
  ...
  MX_I2C1_Init();
  /* USER CODE BEGIN 2 */
    //float  cval=3.14; // check用 shimojo
   //char textdata[24];
    printf("Hello World shimojo \r\n");
    HAL_Delay(1000);
    ...
    //2.  configure accel and gyro parameters
      ...
      myMpuConfig.ClockSource = Internal_8MHz;//8Mhz Clock
      myMpuConfig.CONFIG_DLPF = DLPF_184A_188G_Hz;//184Hz Accsl. 188H Gyro.
      myMpuConfig.Gyro_Full_Scale = FS_SEL_500;//500 deg/sec
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  //Get Raw data
	      ...
	      //data print
	      /*STM32CubeIDEのprintfでfloatが使えない件の対処方法を参照*/
	      /*
	        プロジェクトを右クリック→Properties,
	        C/C++ Build→Settings→MCU Settings,
	        Use float with printf from newlib-nano (-u _printf_float)にチェック
	       */
	      //sprintf (textdata,"%f",myAccelScaled.x);
	       //printf(" %s", textdata);
	       //printf ("%f",myAccelScaled.x); //
	       //printf(" \r\n"); // 改行
	       //printf(" = myAccelScaled \r\n");

	       //add shimojo begin
	  	 MadgwickAHRSupdateIMU(myGyroScaled.x,myGyroScaled.y,myGyroScaled.z,myAccelScaled.x,myAccelScaled.y,myAccelScaled.z);
...
/* USER CODE BEGIN 4 */
// printf用コード　ST-LinkよりTeratermに出力する。通常COM4経由かな。
